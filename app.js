...
function parsePrompt(text){
  ...
  else if(/mai mare|puternic|dramatic/.test(t)) zoom = 0.095;
  else if(/lent|ușor|subtil|încet/.test(t)) zoom = 0.045;

  let rise = /ridic|în sus|upward|rise/.test(t) ? 0.05 : 0;
  if(/foarte ușor|foarte lent|subtil/.test(t)) rise = Math.min(rise, 0.035);

  let pan = 0;
  if(/dreapta|right/.test(t)) pan = 0.025;
  if(/stânga|stanga|left/.test(t)) pan = -0.025;
  ...
  return {
    ...
    vibe: $("vibe").value,
    zoom, rise, pan,
    clouds: !/fără nori|no clouds/.test(t) && /nori|cloud/.test(t),
    water: !/fără apă|no water/.test(t) && /apă|apa|water|râu|rau|river|reflex/.test(t),
    glow: true,
    subjectStill: /băiatul.*(nu|nemișcat|nemiscat|fix)|subject.*still|frozen/.test(t),
    textFixed: /text|scris|citat/.test(t) && /(fix|rămân|raman|final|permanent|nu.*modific)/.test(t),
  };
}
...
function cloudStrength(vibe){
  return vibe === "dramatic" ? 0.05 : vibe === "dreamy" ? 0.04 : 0.03;
}
function waterStrength(vibe){
  return vibe === "dramatic" ? 0.04 : vibe === "dreamy" ? 0.03 : 0.025;
}
function glowAlpha(vibe){
  if(vibe === "dramatic") return 0.06;
  if(vibe === "dreamy") return 0.05;
  if(vibe === "clean") return 0.025;
  return 0.035;
}

function drawCloudEffect(p){
  const W = canvas.width, H = canvas.height;
  const intensity = cloudStrength(cfg.vibe);
  ctx.save();
  ctx.globalCompositeOperation = "screen";
  ctx.globalAlpha = intensity;
  const drift = Math.sin(p*Math.PI*2) * 14;
  ...
  ctx.fillStyle = g;
  ctx.fillRect(-40, H*.05, W+80, H*.28);
  ctx.restore();
}

function drawWaterEffect(p){
  const W = canvas.width, H = canvas.height;
  const intensity = waterStrength(cfg.vibe);
  ctx.save();
  ctx.globalCompositeOperation = "screen";
  ctx.globalAlpha = intensity;
  for(let i=0;i<14;i++){
    ...
    ctx.fillStyle = cfg.vibe === "dramatic" ? "rgba(255,190,120,.78)" : "rgba(255,215,160,.75)";
...
function drawGlow(p){
  ...
  ctx.save();
  ctx.globalCompositeOperation = "screen";
  ctx.globalAlpha = glowAlpha(cfg.vibe) + 0.015*Math.sin(p*Math.PI);
  const color = cfg.vibe === "dramatic" ? "255,160,90" : cfg.vibe === "dreamy" ? "255,205,170" : "255,182,115";
  const g = ctx.createRadialGradient(W*.52,H*.32,0,W*.52,H*.32,W*.52);
  g.addColorStop(0,`rgba(${color},.88)`);
...
function drawFrame(progress){
  ...
  const zoomBase = 1.03;
  const zoom = zoomBase + cfg.zoom*p;
...
$("imageInput").addEventListener("change", e=>{
  ...
  img.onload = ()=>{
    ...
    $("status").textContent = "Imagine încărcată. Poți face preview sau export.";
...
function interpretPrompt(){
  ...
  $("status").textContent = "Robotul a interpretat comanda.";
  $("status").className = "status good";
}
$("interpretBtn").addEventListener("click", interpretPrompt);

["fps","duration","vibe"].forEach(id=>{
...
async function exportVideo(){
  ...
  let recorder;
  try {
    recorder = mime ? new MediaRecorder(stream, { mimeType: mime, videoBitsPerSecond: 12000000 }) : new MediaRecorder(stream);
  } catch(e){
    $("status").textContent = "Browserul nu poate porni exportul. Folosește Screen Recording sau browser desktop.";
    $("status").className = "status warn";
    return;
  }
  ...
  recorder.onerror = ()=>{
    $("status").textContent = "Eroare la export.";
    $("status").className = "status warn";
  };
  recorder.onstop = ()=>{
    ...
    a.href = url;
    a.download = `cinematic_fun_${cfg.fps}fps_${Date.now()}.${ext}`;
    document.body.appendChild(a);
    a.click();
    a.remove();
    setTimeout(()=>URL.revokeObjectURL(url), 5000);
    $("status").textContent = `Export terminat • ${cfg.fps} FPS • ${ext.toUpperCase()}`;
    $("status").className = "status good";
  };
